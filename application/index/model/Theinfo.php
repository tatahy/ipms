...
class Theinfo extends Model
{
    ...
    const THETYPE=conTheEntArr['type'];
    const ENTTITY='thesis';
   ...
   //获取器，获取数据表theinfo中status字段值，转换为中文输出
    ...
    public function assPeriodQuery($period='',$whereArr=[]) {
      ...
      if($auth['read']==1 && $authNum<=1){
        ...
        $scopeArr=($period=='total')?array('id'=>['>',0]):array('status'=>['like','%'.$period.'%']);
        //不能使用已定义的前置查询范围，因为查询的条件是需要在一个前置查询范围内分为2个不同的查询。使用闭包实现
        $query=$this->where(function($query) use($dept,$scopeArr,$whereArr){
                      $query->where($scopeArr)->where('dept',$dept)->where($whereArr);
                    })
                    ->whereOr(function($query) use($dept,$userName,$scopeArr,$whereArr){
                      $query->where($scopeArr)->where('author',$userName)->where('dept','<>',$dept)->where($whereArr);
                    });
...
//                    });
    ...
    #组装$tArr
      ...
      if($field=='type') $tArr=self::THETYPE;
