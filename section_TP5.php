...
举例：查询一个用户列表，然后需要关联查询用户的档案数据。
方式1. 使用`relation`方法：先查询用户列表数据，每个用户再单纯查询档案数据。如果用户列表数据有10个，那么就会产生11次查询。

方式2. 使用`with`方法:最终查询出来的关联数据是一样的，但由于with查询使用的是预载入查询，因此实际只会产生2次查询。

方式3.使用`load`方法则更先进，先查询出用户列表，然后在需要关联数据的时候使用load方法获取关联数据，尤其适合动态关联的情况，最终也是两次查询，因此称为延迟预载入。
